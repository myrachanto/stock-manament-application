...
<router-link to="/customer/invoice/create"><button class="btn btn-info btn-sm">New Invoice</button></router-link>
	...
	<tr v-for="classe in model.data">
		<td>{{ classe.id}}</td>
		<td>{{ classe.customer.name}}/{{ classe.customer.company}}</td>
		<td>{{ classe.due_date}}</td>
		<td>{{ classe.discount}}</td>
		<td>{{ classe.total}}</td>
		<td>
<router-link :to="'/customer/invoice/' + classe.id"><button class="btn btn-info btn-xs">view</span></button></router-link>
 </td>
	...
	data(){
		return{
			...
			thead:[
				...
				{title: 'Customer', key: 'customer_id', sort: true},
			...
			filter: [
				'id', 'customer_id', 'due_date', 'discount', 'total', 'date'
			],
